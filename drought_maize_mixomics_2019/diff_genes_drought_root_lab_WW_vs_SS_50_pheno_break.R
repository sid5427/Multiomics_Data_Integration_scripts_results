...
##upload metabolite
metab_drought <- read.csv('field_metab.txt',header=TRUE,sep='\t',row.names=1, check.names=FALSE)
metab_drought_t <- data.frame(metab_drought)
...
pheno_drought <- read.csv('lab_phenotype_data.txt',header=TRUE,sep='\t',row.names=1, check.names=FALSE)
...
save.image(file = "diff_genes_root_drought_50_work_space.RData")
## ------------------------------------------------------------------------
...
plotVar(sgccda.res, var.names = FALSE, style = 'graphics', legend = TRUE, 
        pch = c(16, 17, 15), cex = c(0.8,0.8,0.8), col = c('darkgreen','brown1','blue')) #, 'lightgreen darkorchid'
?plotVar
...
#           color.cor = c("chocolate3","grey20"), size.labels = 1.0,size.variables = 0.8,ncol.legend = 1)
#dev.off()
...
jpeg('loading_1.jpg',width = 1280, height = 800)
plotLoadings(sgccda.res, comp = 1, contrib = 'max', method = 'median',size.name = 1,size.legend = 1.5,border = TRUE)
dev.off()
...
##custom heatmap
heatmap_1 <- cimDiablo(sgccda.res,margin=c(10,4),legend = FALSE)
...
library("pheatmap")
pheatmap(t(heatmap_1), cutree_rows = 3,cutree_cols = 3,fontsize_row = 8, border_color= "black")
?pheatmap()
dev.off()
...
heatmaply(x=t(heatmap_1),color = plasma,Colv = NULL,hclust_method = 'complete', k_row = NA,dendrogram = 'both')
?heatmaply
